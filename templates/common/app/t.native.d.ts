...
declare global {
    ...
    interface TitanRuntimeUtils {
        ...
        /**
         * HTTP Response Builder — control status codes, headers, content types, and redirects.
         *
         * Returns special `TitanResponse` objects (with `_isResponse: true`) that the
         * Rust/Axum layer interprets to construct the final HTTP response. This is the
         * recommended way to send non-JSON or non-200 responses from actions.
         *
         * > **Note:** If an action returns a plain object/array, Titan automatically
         * > wraps it as a `200 OK` JSON response. Use `t.response.*` only when you
         * > need custom status codes, specific content types, redirects, or extra headers.
         *
         * @example
         * ```js
         * // Return 201 Created
         * export function createItem(req) {
         *   // ... create item ...
         *   return t.response.json({ id: 1, name: "New item" }, 201);
         * }
         *
         * // Return 404 Not Found
         * export function getItem(req) {
         *   const item = findItem(req.params.id);
         *   if (!item) return t.response.json({ error: "Not found" }, 404);
         *   return item;
         * }
         *
         * // Return HTML
         * export function homepage(req) {
         *   return t.response.html("<h1>Welcome to Titan</h1>");
         * }
         *
         * // Redirect
         * export function oldRoute(req) {
         *   return t.response.redirect("/new-route", 301);
         * }
         * ```
         *
         * @see {@link TitanCore.ResponseModule} for all available methods.
         * @see https://titan-docs-ez.vercel.app/docs/04-runtime-apis — Runtime APIs (t.response)
         */
    ...
    namespace TitanCore {
        ...
        /**
         * Standard response object returned by all `t.response.*` methods.
         *
         * This object is intercepted by the Rust/Axum layer to construct
         * the actual HTTP response. The `_isResponse: true` flag signals
         * to the runtime that this is a controlled response rather than
         * a plain data return.
         *
         * You should **not** construct this manually — use the
         * `t.response.json()`, `t.response.text()`, `t.response.html()`,
         * or `t.response.redirect()` helpers instead.
         *
         * @see {@link ResponseModule} for builder methods.
         */
        interface TitanResponse {
            /** Internal flag. Always `true`. Used by the runtime to identify response objects. */
            _isResponse: true;

            /** HTTP status code (e.g., `200`, `201`, `301`, `404`, `500`). */
            status: number;

            /** Response headers as key-value pairs (e.g., `{ "Content-Type": "application/json" }`). */
            headers: Record<string, string>;

            /** The serialized response body string (may be `undefined` for redirects). */
            body?: string;

            /** The redirect target URL (only present for redirect responses). */
            redirect?: string;
        }
         ...
         *
         * Use these helpers to return custom HTTP responses from actions with
         * specific status codes, content types, headers, or redirects.
         *
         * Every method returns a `TitanResponse` object (with `_isResponse: true`)
         * that the Rust/Axum layer interprets to build the final HTTP response.
         *
         * > **Tip:** If your action simply returns a plain object or array, Titan
         * > automatically sends it as `200 OK` with `Content-Type: application/json`.
         * > Use `t.response.*` only when you need something different.
         *
         * @example
         * ```js
         * // JSON with custom status
         * return t.response.json({ error: "Not found" }, 404);
         *
         * // Plain text
         * return t.response.text("OK", 200);
         *
         * // HTML page
         * return t.response.html("<h1>Hello!</h1>");
         *
         * // Redirect with custom headers
         * return t.response.redirect("/dashboard", 302, {
         *   "X-Redirected-By": "Titan"
         * });
         * ```
         *
         * @see https://titan-docs-ez.vercel.app/docs/04-runtime-apis — Runtime APIs (t.response)
        ...
        interface ResponseModule {
             ...
             *
             * The `data` parameter is automatically serialized with `JSON.stringify()`.
             ...
             * @returns A `TitanResponse` object for the runtime.
             *
             * @example
             * ```js
             * // 200 OK (default)
             ...
             *
             * // 201 Created
             * return t.response.json({ id: 1, created: true }, 201);
             *
             * // 400 Bad Request with custom header
             ...
             * ```
             */
            json(data: any, status?: number, extraHeaders?: Record<string, string>): TitanResponse;
             ...
             * ```
             */
            text(content: string, status?: number, extraHeaders?: Record<string, string>): TitanResponse;

            /**
             * Return an HTML response (`Content-Type: text/html`).
             *
             * @param content - The HTML string to send.
             * @param status - HTTP status code. Defaults to `200`.
             * @param extraHeaders - Additional headers to include in the response.
             * @returns A `TitanResponse` object for the runtime.
             *
             * @example
             * ```js
             * return t.response.html("<h1>Welcome</h1><p>Powered by Titan</p>");
             *
             * // Serve a template
             * const template = t.read("./templates/index.html");
             * return t.response.html(template);
             * ```
             */
            html(content: string, status?: number, extraHeaders?: Record<string, string>): TitanResponse;

            /**
             * Return an HTTP redirect response.
             *
             * @param url - The target URL to redirect to (absolute or relative).
             * @param status - HTTP redirect status code. Defaults to `302` (Found / Temporary).
             ...
             * return t.response.redirect("/dashboard");
             *
             ...
             * ```
             */
            redirect(url: string, status?: number, extraHeaders?: Record<string, string>): TitanResponse;
        }
    }
}
